...
public class CloseAuto extends LinearOpMode {
    ...
    public void buildPaths() {
        MOVETOLAUNCH = follower
                ...
                .setLinearHeadingInterpolation(Math.toRadians(180-angleOffset), Math.toRadians(Math.abs(angleOffset-53)))
                .build();

        MOVETOCOLLECT = follower
                .pathBuilder()
                .addPath(new BezierLine(autoPoses.launchPose, autoPoses.readyPickUp1))
                .setLinearHeadingInterpolation(Math.toRadians(Math.abs(angleOffset-53)), Math.toRadians(angleOffset))
                .build();
        ...
        MOVETOLAUNCH2 = follower
                ...
                .setLinearHeadingInterpolation(Math.toRadians(angleOffset), Math.toRadians(Math.abs(angleOffset-45)))
                .build();

        ENDOFFLINE = follower
                .pathBuilder()
                .addPath(new BezierLine(autoPoses.launchPose, autoPoses.endOffLine))
                .setLinearHeadingInterpolation(Math.toRadians(Math.abs(angleOffset-45)), Math.toRadians(270))
                .build();
    }
